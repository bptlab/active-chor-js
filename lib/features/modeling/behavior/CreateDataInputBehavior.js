  ...
  this.preExecute('shape.create', function(event) {
    ...
    if (is(shape, 'bpmn:DataInput')) {
      ...
      if (!parent.ioSpecification) {
        ...
        // with reverts and redos. Also we do not care if the ioSpec is left over by a revert and empty.

      }

      shape.businessObject.id = moddle.ids.nextPrefixed('DataInput_', shape.businessObject);
      // This is just a temporary id which will be overwritten.
      // We need to add a temporary ID so updateProperty does not brake on revert.

      // The DI id, however, is not temporary.
