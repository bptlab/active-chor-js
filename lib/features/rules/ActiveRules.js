...
import { isAnyAndNotLabel } from 'chor-js/lib/util/DiagramWalkerUtil';
...
ActiveRules.prototype.canConnect = function(source, target, connection) {
  // special handling for data associations, we do not want Choreo Tasks.
  if (isAny(source, ['bpmn:DataObjectReference', 'bpmn:DataInput']) &&
    isAnyAndNotLabel(target, 'bpmn:SubChoreography', 'bpmn:CallChoreography', 'bpmn:ScriptTask') &&
    source.parent !== target) {
    return { type: 'bpmn:DataInputAssociation' };
  }

  if (isAny(source, ['bpmn:SubChoreography', 'bpmn:CallChoreography', 'bpmn:ScriptTask']) &&
    isAnyAndNotLabel(target, 'bpmn:DataObjectReference', 'bpmn:DataOutput') &&
