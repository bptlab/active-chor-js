...
export default class ActiveTreeWalker extends ChoreoTreeWalker {
  ...
  handleIOSpecification(ioSpecification, parentShape) {
    ...
    if (ioSpecification.dataOutputs) {
      ioSpecification.dataOutputs.forEach(input => this.visitIfDi(input, parentShape));
    }
  ...
  handleFlowElements(flowElements, parentShape) {
    forEach(flowElements, e => {
      ...
      } else if (isAny(e, ['bpmn:DataObjectReference', 'bpmn:DataInput', 'bpmn:DataOutput'])) {
    ...
    flowElements = filter(
      flowElements,
      x => !isAny(x, ['bpmn:DataObject', 'bpmn:DataObjectReference', 'bpmn:DataInput', 'bpmn:DataOutput']));
